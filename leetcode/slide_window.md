# 滑动窗口解析
## 概述
滑动窗口常用来解决求字符串子串问题，借助 map 和计数器，其能在 O(n) 时间复杂度求子串问题。

滑动窗口和双指针有些类似，可以理解为往同一个方向走的双指针，是双指针的复杂应用。

unordered_map 可以实现 O(1) 查询，使用 count(key) 方法判断 key 是否存在；使用中括号 map[key] 访问不存在的 key 时，c++ 默认会创建这个 key，并赋值为 0 。
...
while (r < s.size()) {
    window.add(s[r++]);         // 右指针右移扩充窗口
    
    while (l < r && valid) {    // 条件满足时，左指针右移收缩窗口
        window.remove(s[l]);    // 出窗
        l++;
    }
    update_result();            // 条件满足时，更新返回结果
}
```
